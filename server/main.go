...
import (
	...
	"github.com/yageek/glmf-go-api/authentication"
...
func main() {
	...
	r := gin.Default()
	// URLs ne nécessitant pas d'authentification
	r.POST("/login", authentication.Login)

	r.GET("/news", GetAllNews)
	r.GET("/news/:id", newsIDMiddleware, GetNews)

	// URL nécessitant pas d'authentification
	r.PUT("/news", authentication.JWTMiddleware, CreateNews)
	r.POST("/news/:id", authentication.JWTMiddleware, newsIDMiddleware, UpdateNews)
	r.DELETE("/news/:id", authentication.JWTMiddleware, newsIDMiddleware, DeleteNews)
	r.Run()
}
